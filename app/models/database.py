...
def init_db() -> None:
    ...
    with get_db_connection() as conn:
        ...
        conn.execute(
            ...
            "idx_bom_items_project ON bom_items(project_name)"
        )
        conn.commit()
...
def get_projects() -> List[Dict[str, Any]]:
    ...
    with get_db_connection() as conn:
        cursor = conn.execute(
            "SELECT name, total_files FROM projects ORDER BY name ASC"
        )
...
def save_bom_data_for_project(
    project_name: str, items: List[Dict[str, Any]], total_files_processed: int
) -> None:
    """Save new BOM items for a project.

    Overwrites only matching drawing entries and updates
    files count.
    """
    with get_db_connection() as conn:
        ...
        # Clear items only for the drawings in the new upload
        # (prevent duplicates while appending)
        drawings_to_overwrite = list(
            set(item.get("drawing") for item in items if item.get("drawing"))
        )
        ...
        for item in items:
            conn.execute(
                ...
                INSERT INTO bom_items (
                    project_name, sub_project, drawing,
                    category, pno, description,
                    size, material, standard,
                    qty, qty_unit, weight, weight_unit
                ) VALUES (
                    ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?
                )
            """,
        ...
        cursor = conn.execute(
            "SELECT COUNT(DISTINCT drawing) FROM bom_items WHERE project_name = ?",
            (project_name,),
        )
...
def get_bom_data_for_project(project_name: str) -> Tuple[List[Dict[str, Any]], int]:
    ...
    with get_db_connection() as conn:
        ...
        cursor = conn.execute(
            """
            SELECT sub_project, drawing, category,
                   pno, description, size, material,
                   standard, qty, qty_unit,
                   weight, weight_unit
            FROM bom_items
            WHERE project_name = ?
            ORDER BY id ASC
        ...
        # Get files count
        cursor = conn.execute(
            "SELECT total_files FROM projects WHERE name = ?", (project_name,)
        )
        row = cursor.fetchone()
...
def clear_project_data(project_name: str) -> None:
    ...
    with get_db_connection() as conn:
        ...
        conn.execute(
            "UPDATE projects SET total_files = 0 WHERE name = ?", (project_name,)
        )
        conn.commit()
...
def clear_sub_project_data(project_name: str, sub_project: str) -> None:
    ...
    with get_db_connection() as conn:
        conn.execute(
            "DELETE FROM bom_items WHERE project_name = ? AND sub_project = ?",
